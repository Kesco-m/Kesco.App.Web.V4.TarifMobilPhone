...
namespace Kesco.App.Web.TarifMobilPhone
{
    ...
    /// </summary>
    public partial class Details_Old : Page
    {
        ...
        /// </summary>
        public Dictionary<string, Type> _columnList;

        /// <summary>
        /// Используется для работы со списком данных
        /// </summary>
        public DataView _dvData;

        /// <summary>
        /// Словарь колонками, участвующими в группировке
        /// </summary>
        public Dictionary<string, bool> _groupByList;
        ...
        /// Очередь сортировки
        /// </summary>
        public SpecialQueue<string> _sqSort;

        /// <summary>
        /// Словарь с колонками сумм
        /// </summary>
        public Dictionary<string, decimal> _sumList;
        ...
        private void Print()
        {
            ...
            _dvData = ((Details_Old) p)._dvData;
            _qsParams = ((Details_Old)p)._qsParams;
            _sqSort = ((Details_Old)p)._sqSort;
           ...
           w.Write("<br/><br/>");
           w.Write(RenderTable(_dvData, true).ToString());
        ...
        private StringWriter RenderTable(DataView dv, bool print=false)
        {
            var w = new StringWriter();
        ...
        private void RenderTableHeader(StringWriter w, DataView dv)
        {
            ...
            if (isPrintPage)
                w.Write(RenderSortableColumnLink("Роуминг", "Роуминг"));
            else
            {
            ...
            w.Write(@"<td noWrap valign='middle'>");
            if (isPrintPage)
                w.Write(RenderSortableColumnLink("Телефон", TM_FPhone));
            else
            {
            ...
            if (isPrintPage)
                w.Write(RenderSortableColumnLink("Тип", TMD_TblColumn2));
            else
            {
            ...
            w.Write(@"<td valign='middle'>");
            if (isPrintPage)
                w.Write(RenderSortableColumnLink("Услуга", TMD_TblColumn5));
            else
            {
            ...
            w.Write(@"<td noWrap valign='middle'>");
            if (isPrintPage)
                w.Write(RenderSortableColumnLink("Описание", "Описание услуги"));
            else
            {
        ...
        private void RenderTableBody(StringWriter w, DataView dv, ref decimal calcSum, ref decimal calcSumE)
        {
            ...
            if (!isPrintPage)
                title = GetTitleByColor();

            for (int i = 0; i < dv.Count; i++)
            {
                w.Write("<tr");
                if (i%2 != 0 && !isPrintPage)
                ...
                w.Write("<td class='tdR' noWrap style='{0}' {1}>", isPrintPage?"":_qsParams["Color"], title);
                ...
                w.Write("</td>");
                w.Write("<td class='tdR' style='{0}' {1}>", isPrintPage ? "" : _qsParams["Color"], title);
