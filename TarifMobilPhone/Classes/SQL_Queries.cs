...
";
        ...
        public const string SQL_ДоговораПоКоторымБылаТарификация =
            @"
SELECT КодДокумента КодДоговора, Д.Исполнитель + ' - ' + Д.Заказчик + ISNULL(' №' + NULLIF(Д.НомерДокумента,''),'') Договор 
FROM Инвентаризация.dbo.ДоговораСвязи Д (nolock) 
WHERE EXISTS(
SELECT * FROM Тарификация.dbo.vwТарификацияСотовыхИтоги Т(nolock) 
WHERE Т.КодДоговора = Д.КодДокумента
...
(       SELECT 	Т.КодДоговора,
                ...
                У.НДСВТарификации,
                У.НДСНаСумму,
                Д.Исполнитель + ' - ' + Д.Заказчик + ISNULL(' №' + NULLIF(Д.НомерДокумента,''),'') Договор,
                Д.КодЛицаЗаказчика,
		...
		Инвентаризация.dbo.ДоговораСвязи  Д  (nolock) ON Т.КодДоговора = Д.КодДокумента LEFT JOIN 
        ...
        GROUP BY	Т.КодДоговора, ISNULL(У.КодВалюты, -1), У.ОкруглениеСуммы, У.СтавкаНДС, У.НДСВТарификации, У.НДСНаСумму, Д.Исполнитель, Д.Заказчик, Д.НомерДокумента, Д.КодЛицаЗаказчика,
...
SELECT * FROM 
    ...
    WHERE ((@OpenMonth = 1 AND Т.Год = 0 AND Т.Месяц = 0)
			    OR (@OpenMonth = 0 AND ((@Год IS NULL OR Т.Год = @Год) AND (@Месяц IS NULL OR Т.Месяц = @Месяц))))
	    AND (@КодДоговора IS NULL OR Т.КодДоговора = @КодДоговора)
        AND (ISNULL(@Договор,'') = '' OR  КодДоговора IN (SELECT КодДокумента FROM Инвентаризация.dbo.ДоговораСвязи WHERE (ДоговораСвязи.Исполнитель + ' - ' + ДоговораСвязи.Заказчик + ISNULL(' №' + NULLIF(ДоговораСвязи.НомерДокумента,''),'')) LIKE '%' + @Договор + '%'))
