...
namespace Kesco.App.Web.TarifMobilPhone
{
    ...
    public partial class DefaultPage : Page
    {
        ...
        */
        ...
        {
            var year = cbYear.Value.Length == 0 ? "-1" : cbYear.Value;
            var month = cbMonth.Value.Length == 0 ? "-1" : cbMonth.Value;
            var dogovor = cbDogovor.Value.Length == 0 ? "-1" : cbDogovor.Value;
            ...
            var url = Config.uri_Report + "?/Accounting/AccountingMobil" + p +
        ...
        {
            var disp = p.Equals("1") ? false : true;
            Display("tr2", disp);
            Display("tr3", disp);
        }
        ...
        private void FillComboBoxYear()
        {
            ...
            var dt = DBManager.GetData(SQL_Queries.SQL_ПервыйГодТарификации, Config.DS_accounting_phone,
        ...
        private void FillComboBoxMonth()
        {
            ...
            cbMonth.Value = "";
            var month = "";
            var culture = new CultureInfo(CurrentUser.Language);
            for (var i = 1; i < 13; i++)
            {
        ...
        private void FillComboBoxDogovor()
        {
            ...
            var sqlParams = GetSQLParams(1);
            var dt = DBManager.GetData(SQL_Queries.SQL_ДоговораПоКоторымБылаТарификация,
        ...
        private void FillComboBoxEmployee()
        {
            ...
            var sqlParams = GetSQLParams(2);
            var dt = DBManager.GetData(SQL_Queries.SQL_СотрудникиПоКоторымБылаТарификация,
                Config.DS_accounting_phone, CommandType.Text, sqlParams);
        ...
        private void FillComboBoxPhone()
        {
            ...
            var sqlParams = GetSQLParams(3);
            var dt = DBManager.GetData(SQL_Queries.SQL_ТелефоныПоКоторымБылаТарификация,
                Config.DS_accounting_phone, CommandType.Text, sqlParams);
        ...
        {
            var sqlParams = GetSQLParams(-1);
            var dt = DBManager.GetData(SQL_Queries.SQL_ТарификацияИтоги, Config.DS_accounting_phone,
            ...
            if (dt.Rows.Count == 0)
            {
                ...
                JS.Write(
                    "var objEmpty = document.getElementById('divEmpty'); if (objEmpty) objEmpty.style.display='block';");
                RestoreCursor();
                return;
            }
            ...
            JS.Write(
                "var objEmpty = document.getElementById('divEmpty'); if (objEmpty) objEmpty.style.display='none';");
        ...
        private Dictionary<string, object> GetSQLParams(int inx)
        {
            ...
            if (inx != 0)
            {
                ...
                else if (!int.TryParse(cbYear.Value, out y))
                    sqlParams.Add("@Год", -1);
                else
        ...
        private void RenderTableHeader(StringBuilder sb)
        {
            ...
            sb.AppendFormat("<td colspan='5'>{0}</td>",
                cbMonth.Value.Length > 0 || chOpenMonth.Value.Equals("1")
                    ? HttpUtility.HtmlEncode(TM_FDogovor)
                    : HttpUtility.HtmlEncode(TM_FMonth));
            sb.AppendFormat("<td>{0}</td>", HttpUtility.HtmlEncode(TM_TblColumn2));
            sb.AppendFormat("<td>{0}</td>", HttpUtility.HtmlEncode(TM_TblColumn3));
            sb.AppendFormat("<td>{0}<br>{1}</td>", HttpUtility.HtmlEncode(TM_TblColumn4_0),
                HttpUtility.HtmlEncode(TM_TblColumn4_1));
        ...
        private void RenderTableHeaderItogo(DataTable dt, DataTable dtNDS, StringBuilder sb)
        {
            ...
            {
                var currencyRow = (from m in _currencies.AsEnumerable()
                ...
                sb.AppendFormat("<td style='text-align:right;'>{0}</td>", Utils.Convert.Second2TimeFormat(d.Секунд));
        ...
        private void RenderTableItemsLevelFirst(DataTable dt, DataTable dtNDS, StringBuilder sb)
        {
            ...
            var color = "";
            var title = "";
            var ndsCalc = СalculationNDS.Unknown;
            var sbNextLevel = new StringBuilder();
            DataRow rowNDS;
        ...
        /// <param name="sb">HTML-контент</param>
        private void RenderTableItemsLevelTwo(DataTable dt, int d, string dText, string currency,
            СalculationNDS ndsCalc,
            ...
            string trId, StringBuilder sb, ref decimal calcSum, ref decimal calcSumE)
        {
            ...
            foreach (var e in results)
            {
                ...
                sb.AppendFormat("<td style='text-align:right;'>{0}</td>", Utils.Convert.Second2TimeFormat(e.Секунд));
                sb.AppendFormat("<td style='text-align:right;{2}' {3}>{0}{1}</td>", sum.ToString("N2"), currency, color,
                    title);
                sb.AppendFormat("<td style='text-align:right;{2}' {3}>{0}{1}</td>", sumE.ToString("N2"), currency,
                    color,
                    title);
                sb.Append("</tr>");
            ...
            string color, string title, string trId, StringBuilder sb, ref decimal calcSum, ref decimal calcSumE)
        {
            ...
            var pOpenMonth = chOpenMonth.Value.Length == 0 ? "0" : chOpenMonth.Value;
            var pYear = pOpenMonth.Equals("1") ? "0" : cbYear.Value;
            var pMonth = pOpenMonth.Equals("1") ? "0" : cbMonth.Value;
            var pDogovor = d.ToString();
            var pDogovorT = HttpUtility.HtmlEncode(dText);
            var pPhone = "";
            ...
            foreach (var ph in results)
            {
                ...
                sumE = ndsCalc.Equals(СalculationNDS.OnCostOfEachCall)
                ...
                sb.AppendFormat(@"<td colspan=2><span {1}>{0}</span></td>", ph.Абонент,
                    ph.Подчинённый.Equals(1)
                        ...
                        : "");
                sb.AppendFormat("<td style='text-align:right;'>{0}</td>", Utils.Convert.Second2TimeFormat(ph.Секунд));
                sb.AppendFormat("<td style='text-align:right;{2}' {3}>{0}{1}</td>", sum.ToString("N2"), currency, color,
                    title);
                sb.AppendFormat("<td style='text-align:right;{2}' {3}>{0}{1}</td>", sumE.ToString("N2"), currency,
                    color,
        ...
        private void RenderTableItemsAllMonthsLevelFirst(DataTable dt, DataTable dtNDS, StringBuilder sb)
        {
            ...
            {
                sec = rs.Секунд;
                var updateRow = dtSum.AsEnumerable()
                    .Where(r => r.Field<int>("Месяц") == rs.Месяц && r.Field<int>("КодВалюты") == rs.КодВалюты)
                    .SingleOrDefault();
        ...
        /// <param name="sb">HTML-контент</param>
        private void RenderTableItemsAllMonthsLevelTwo(DataTable dt, DataTable dtNDS, DataTable dtSum, int m,
            string trId, StringBuilder sb)
        {
            var results = from r in dt.AsEnumerable()
            ...
            var calcNds = СalculationNDS.Unknown;
            var sbNextLevel = new StringBuilder();
            DataRow rowNDS;

            foreach (var d in results)
            {
                var currencyRow = (from cur in _currencies.AsEnumerable()
                ...
                var updateRow = dtSum.AsEnumerable()
                    .Where(r => r.Field<int>("Месяц") == d.Месяц && r.Field<int>("КодВалюты") == d.КодВалюты)
                    .SingleOrDefault();
                if (updateRow != null)
                {
                    updateRow.SetField("Сумма", (decimal) updateRow["Сумма"] + sum);
                ...
                sb.AppendFormat("<td colspan=3 {1}>{0}</td>", d.Договор,
                    d.КодДоговора < 1 ? "style='color:gray;'" : "");
                sb.AppendFormat("<td style='text-align:right;'>{0}</td>", Utils.Convert.Second2TimeFormat(d.Секунд));
                sb.AppendFormat("<td style='text-align:right;{2}' {3}>{0}{1}</td>", sum.ToString("N2"), currency, color,
                    title);
                sb.AppendFormat("<td style='text-align:right;{2}' {3}>{0}{1}</td>", sumE.ToString("N2"), currency,
        ...
        /// <param name="sb">HTML-контент</param>
        private void RenderTableItemsAllMonthsLevelThree(DataTable dt, int d, int m, string currency,
            СalculationNDS calcNds,
            decimal? ndsStavka, int? ndsIn, int? scale,
            string trId, StringBuilder sb, ref decimal calcSum, ref decimal calcSumE)
        {
            var results = from r in dt.AsEnumerable()
            ...
            foreach (var u in results)
            {
                ...
                sb.AppendFormat("<td style='text-align:right;'>{0}</td>", Utils.Convert.Second2TimeFormat(u.Секунд));
                sb.AppendFormat("<td style='text-align:right;{2}' {3}>{0}{1}</td>", sum.ToString("N2"), currency, color,
                    title);
                sb.AppendFormat("<td style='text-align:right;{2}' {3}>{0}{1}</td>", sumE.ToString("N2"), currency,
                    color,
        ...
        private void ItemsAllMonthsLevelFour(DataTable dt, int d, int m, string pl, СalculationNDS calcNDS,
            decimal? ndsStavka,
            ref decimal calcSum, ref decimal calcSumE)
        {
            var results = from r in dt.AsEnumerable()
                where r.Field<int>("КодДоговора") == d && r.Field<int>("Месяц") == m &&
                      r.Field<string>("Пользователь") == pl
                group r by
                    new
                    {
                        Абонент = r.Field<string>("Абонент")
            ...
            foreach (var ph in results)
            {
                calcSum += calcNDS.Equals(СalculationNDS.OnCostOfEachCall)
                    ? ph.Сумма * (1 + ndsStavka.Value)
                    : ph.Сумма;
        ...
        {
            var sbSec = new StringBuilder();
            var sbSum = new StringBuilder();
            var sbSumE = new StringBuilder();
            IEnumerable<DataRow> rows = dtSum.AsEnumerable().Where(r => r.Field<int>("Месяц") == month)
                .OrderBy(r => r.Field<int>("КодВалюты"));
            var style = "style='BORDER-bottom:1px solid #efefef;'";
            var styleAlign = "style='text-align:right;' class='gridHeader'";
            ...
            foreach (var r in rows)
            {
                ...
                sbSec.AppendFormat("<div {1}>{0}</div>", Utils.Convert.Second2TimeFormat(r.Field<int>("Секунд")),
                    inx != cnt ? style : "");
                sbSum.AppendFormat("<div {2}>{0}{1}</div>", r.Field<decimal>("Сумма").ToString("N2"), currency,
                    inx != cnt ? style : "");
                sbSumE.AppendFormat("<div {2}>{0}{1}</div>", r.Field<decimal>("СуммаСотрудника").ToString("N2"),
                    currency,
                    inx != cnt ? style : "");
                inx++;
            }
        ...
        {
            var args = data.Split('_');
            var sqlParams = new Dictionary<string, object>();
            var startDate = new DateTime(int.Parse(cbYear.Value), int.Parse(cbMonth.Value), 1);
            var endDate = new DateTime(int.Parse(cbYear.Value), int.Parse(cbMonth.Value),
